export const AwModal = {
    back: 'Go Back',
    close: 'Close Window (ESC)'
}

export const AwCalendar = {
    weekdaysShort: ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'],
    months: [
        'January',
        'February',
        'March',
        'April',
        'May',
        'June',
        'July',
        'August',
        'September',
        'October',
        'November',
        'December'
    ]
}

export const AwDate = {
    format: 'DD.MM.YYYY'
}

export const AwCropper = {
    upload: 'Change Image',
    rotate: 'Rotate 90°',
    save: 'Save Image',
    cancel: 'Cancel'
}

export const AwCodeSnippet = {
    copy: 'Copy'
}

export const AwLayoutDefault = {
    caption:
        '<span class="text-lg md:text-3xl"><strong>Awes.io</strong></span>',
    toggleMenu: 'Toggle mobile menu',
    shrinkMenu: 'Shrink menu',
    toggleSubmenu: 'Toggle submenu',
    footer:
        '© 2019 - Proudly powered on <a href="https://www.awes.io" target="_blank" class="link">Awes.io Platform</a>'
}

export const AwLayoutError = {
    message: 'Something is wrong.',
    subtitle: 'Error'
}

export const AwLayoutFrameCenter = {
    footer:
        '© 2019 - Proudly powered on <a href="https://www.awes.io" target="_blank" class="link">Awes.io Platform</a>'
}

export const AwMarkdownEditor = {
    editor: 'Write',
    preview: 'Preview',
    heading: 'Heading',
    bold: 'Bold',
    italic: 'Italic',
    quote: 'Quote'
}

export const AwMultiBlockBuilder = {
    title: '{title} #{iteration}',
    add: '+ Add {title}',
    remove: 'Remove {title}',
    save: 'Save'
}

export const AwPagination = {
    info: 'Show {from}-{to} of {total}',
    page: 'Page {page} of {pagesTotal}',
    limit: '{limit} per page'
}

export const AwSelect = {
    notFound: 'Nothing found'
}

export const AwTableBuilder = {
    loading: 'Loading...',
    empty: 'There are no data to show'
}

export const AwTimelineBuilder = {
    more: 'Show more'
}

export const AwUploader = {
    drop: 'Drag and drop files here',
    format: 'Supports only {formatString} file formats',
    size: 'Maximum file fize {sizeString}',
    remove: 'Remove file',
    loading: 'Uploading',
    loaded: 'Finished',
    errorExtension: 'File {fileName} has wrong extension',
    errorSize: 'File {fileName} is too big'
}

export const AwSearch = {
    text: 'Search'
}

export default {
    AwModal,
    AwCalendar,
    AwCropper,
    AwCodeSnippet,
    AwDate,
    AwLayoutDefault,
    AwLayoutError,
    AwLayoutFrameCenter,
    AwMarkdownEditor,
    AwMultiBlockBuilder,
    AwPagination,
    AwSelect,
    AwTableBuilder,
    AwTimelineBuilder,
    AwUploader,
    AwSearch
}
